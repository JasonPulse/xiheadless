...
public sealed class CraftBrain(IPerception p, IAuctionHouse ah, ICrafting craft, IZoning zoning, ILifecycle lifecycle, IInventory inv, IShop shop, INavigation nav) : IBrain
{
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        Log.Info($"[craft] char='{p.World.MyName}' gil={p.World.Gil} zone={zoning.CurrentZone} — recipe: {Crystal}+{Ingredient} (Bronze Sheet)");
        ...
        {
            Log.Info($"[craft] no AH in zone {zoning.CurrentZone} — walking to {AhZone}");
            if (!await zoning.GoTo(AhZone, ct)) { Log.Info($"[craft] failed to reach {AhZone} (in {zoning.CurrentZone}) — logging out"); lifecycle.Logout(); return; }
            await Task.Delay(2000, ct);   // let inventory/state resettle after the zone change
        }

        int startGains = p.World.SkillGains[RecipeSkill];
        Log.Info($"[craft] Smithing level {p.World.SkillLevel(RecipeSkill)} at start");

        int made = 0;
        for (int n = 0; n < MaxSynths && !ct.IsCancellationRequested; n++)
        {
            int gainsBefore = p.World.SkillGains[RecipeSkill];
            if (!await EnsureItem(Crystal, ct))    { Log.Info($"[craft] could not buy crystal {Crystal} from AH — stopping"); break; }
            if (!await EnsureItem(Ingredient, ct)) { Log.Info($"[craft] could not buy ingredient {Ingredient} from AH — stopping"); break; }

            byte cSlot = (byte)inv.SlotOf(Crystal), iSlot = (byte)inv.SlotOf(Ingredient);   // EnsureItem guarantees both present
            Log.Info($"[craft] synth #{n + 1}: crystal {Crystal}@{cSlot} + {Ingredient}@{iSlot}");
            ...
            if (res == 13) { Log.Info("[craft] server: must wait longer — retrying"); await Task.Delay(3000, ct); n--; continue; }
            if (res == 0) made++;
            ...
            Log.Info($"[craft] synth #{n + 1} -> {outcome} (made {made}){(upThisSynth > 0 ? $"; Smithing +{upThisSynth / 10.0:0.0}" : "")}");
        }

        double gained = (p.World.SkillGains[RecipeSkill] - startGains) / 10.0;
        Log.Info($"[craft] done — {made}/{MaxSynths} succeeded; Smithing +{gained:0.0} this session (now level {p.World.SkillLevel(RecipeSkill)}); logging out");
        lifecycle.Logout();
    }
