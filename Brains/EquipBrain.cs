...
public sealed class EquipBrain(IPerception p, IGear gear) : IBrain
{
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        Console.WriteLine($"[equip] inventory={p.World.Inventory.Count} items: " +
            string.Join(", ", p.World.Inventory.Select(kv => $"item{kv.Value}@c{kv.Key.container}s{kv.Key.slot}")));
        Console.WriteLine($"[equip] hasOnionSword(16534)={gear.HasItem(16534)}");
        Console.WriteLine($"[equip] skills: H2H={gear.SkillLevel(1)} Sword={gear.SkillLevel(3)} Axe={gear.SkillLevel(5)} GSword={gear.SkillLevel(4)}");
        const uint sword = 16534;   // Onion Sword
        bool ok = await gear.EquipItem(sword, 0, ct);   // 0 = SLOT_MAIN
        Console.WriteLine($"[equip] equip item {sword} to main hand: sent={ok}");
