...
public sealed class Quests(IEvents events) : IQuests
{
    ...
    {
        if (!await events.Examine(npcId, ct)) { Log.Info($"[quest] npc 0x{npcId:X} opened no event"); return false; }
        foreach (var sel in selections)
        {
            ...
            if (!events.EventActive)
            {
                ...
                if (!events.EventActive) { Log.Info("[quest] no further prompt — dialog done"); break; }
            }
